...
def extract_top_value(map_matrix, retain_percent): 
    '''\
...
def project_cell_to_spot(adata, adata_sc,retain_percent):
    '''\
    ...
    # map_matrix = adata.obsm['map_matrix'] * 100  # 放大映射矩阵的值
    ...
    # rename cell types
    cell_type = list(adata_sc.obs['celltype'].unique())
    cell_type = [str(s) for s in cell_type]
    cell_type.sort()
